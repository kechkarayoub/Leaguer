...
void main() {
  ...
  test('Initial storage state not empty', () async {
    ...
    final userSession = await storageService.get('user_session');
    expect(currentLanguage, defaultLanguage);
    expect(userSession, null);
  });

  test('Saving and retrieving a value works correctly', () async {
    await storageService.set(key: 'user_session', obj: {"id": 123, "name": "Ayoub"});
    final result = await storageService.get('user_session');
  ...
  test('Removing a key deletes the stored value', () async {
    await storageService.set(key: 'user_session', obj: {"id": 123});
    await storageService.remove(key: 'user_session');
    final result = await storageService.get('user_session');
  ...
  test('StorageNotifier updates when a value is changed', () async {
    await storageService.set(key: 'user_session', obj: {"id": 123}, updateNotifier: true);
    
    expect(storageService.storageNotifier.value['user_session'], {"id": 123});
  });
}
