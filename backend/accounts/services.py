...
class UserService:
    ...
    @staticmethod
    def generate_unique_username(email="", first_name="", last_name=""):  # pylint: disable=too-many-branches
        """
        ...
        if candidates:
            exists = set(User.objects.annotate(username_lower=Lower('username')).filter(
            ...
            if len(exists) == len(candidates):
                ...
                exists = set(User.objects.annotate(username_lower=Lower('username'))
...
class EmailVerificationService:
    ...
    def send_verification_email(user, language=None):
        ...
        try:
            ...
            raise EmailSendingException(f"Failed to send verification email: {str(e)}") from e
    ...
    def verify_email_token(uid, token):
        ...
        try:
            ...
            raise TokenValidationException(f"Invalid user: {exc}") from exc
        except Exception as e:
    ...
    def verify_phone_code(user, code):
        ...
        try:
            ...
            if user.user_phone_number_verification_code_timestamp:
                expiry_time = user.user_phone_number_verification_code_timestamp + datetime.timedelta(minutes=settings.NUMBER_MINUTES_BEFORE_PHONE_NUMBER_VERIFICATION_CODE_EXPIRATION)  # pylint: disable=line-too-long
                if now() > expiry_time:
