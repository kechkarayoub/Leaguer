...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        expired_outstanding = OutstandingToken.objects.filter(created_at__lt=cutoff_date)
        expired_blacklisted = BlacklistedToken.objects.filter(token__created_at__lt=cutoff_date)
        ...
        self.stdout.write(f"Total tokens to remove: {total_count}")
        
        # Confirmation
        if not options['force']:
            confirm = input(f"Are you sure you want to remove {total_count} expired tokens older than {days} days? [y/N]: ")
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ Removed {blacklisted_deleted} expired blacklisted tokens")
            )
            self.stdout.write(
                self.style.SUCCESS(f"✓ Removed {outstanding_deleted} expired outstanding tokens")
            )
            self.stdout.write(
                self.style.SUCCESS(f"Total tokens cleaned up: {blacklisted_deleted + outstanding_deleted}")
            )
            
            logger.info(f"Token cleanup completed: {blacklisted_deleted + outstanding_deleted} tokens removed")
